...
public abstract class Node implements AgentStorage {
	...
	/**
	 * Gets an login for the user for the given id.
	 * 
	 * @param id
	 * @return agent login
	 * @throws AgentNotKnownException
	 */
	public String getLoginForAgentId ( long id ) throws AgentNotKnownException {
		if ( activeUserList == null) {
			loadUserList();
		}
		...
		try {
			return activeUserList.getContent( UserAgentList.class).getLoginName(id);
		} catch (AgentNotKnownException e) {
			// retry once
			loadUserList();
