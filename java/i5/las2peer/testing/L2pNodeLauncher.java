...
public class L2pNodeLauncher {
	...
	private final static String DEFAULT_WEBCONNECTOR_LOGFILE = "webConnector.log";
	...
	/**
	 * start the web connector at the given port
	 * 
	 ...
	 * @param logFilePath 
	 */
	public void startWebConnector ( String useHttps, String port , String logFilePath) {
		boolean https=useHttps.equals("true");
		startWebConnector(https, Integer.valueOf ( port ), logFilePath );
	}
	/**
	 * start the web connector at the given port
	 * 
	 ...
	 * @param logFilePath
	 */
	public void startWebConnector ( String port , String logFilePath) {
		startWebConnector(true, Integer.valueOf ( port ), logFilePath );
	}
	/**
	 * start the web connector at the given port
	 * 
	 ...
	 * @param port
	 */
	public void startWebConnector ( String port ) {
		startWebConnector(true, Integer.valueOf ( port) ,DEFAULT_WEBCONNECTOR_LOGFILE );
	}
	/**
	 * start the web connector at the standard port (8080)
	 * 
	 * be aware: all method called after the connector (at this node) will 
	 * be started after the connector has been closed!
	 */
	public void startWebConnector () {
		startWebConnector (true, 8080 , DEFAULT_WEBCONNECTOR_LOGFILE);
	}
	 ...
	 * @param iPort
	 */
	public void startWebConnector ( final boolean useHttps, final int iPort, final String logFilePath ) {
		
		try {
			printMessage( "Starting Web Connector!");
			final WebConnector connector = new WebConnector ();
			if(useHttps)
			{
				connector.setHttpsPort( iPort );
				connector.setHttps();
			}
			else
			{
			...
			connector.setLogFile(logFilePath);
			...
			Thread reminder = new Thread ( new Runnable () {
				...
				public void run() {
					try {
						while ( true ) {
							System.out.println("--- web connector still running at port " + iPort +" ---" );
								Thread.sleep( 10000 );
						}
					...
					printMessage ( "--> web connector stopped!");
