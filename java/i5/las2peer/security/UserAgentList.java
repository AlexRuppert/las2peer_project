...
public class UserAgentList implements Serializable {
	...
	private Hashtable<Long, String> htUserIds2Login = new Hashtable<Long, String> ();
	...
	private void removeUser ( UserAgent user ) {
		...
		if ( userEntry != null ) {
			if ( userEntry[0] != null )
			{
				Long id=htLogin2UserIds.get(userEntry[0]);
				htUserIds2Login.remove(id);
	...
	public void updateUser ( UserAgent user ) throws DuplicateEmailException, DuplicateLoginNameException {
		...
		if ( user.hasLogin() )
		{
			...
			htUserIds2Login.put(user.getId(), user.getLoginName());
		}
	 ...
	 */
	public String getLoginName(long id) throws AgentNotKnownException {
		String result = htUserIds2Login.get ( id );
		if ( result != null)
			return result;
		
		throw new AgentNotKnownException("No agent registered for id: " + id );
	}
	
	/**
	 * get the login of the {@link UserAgent} assigned to the id
	 * 
	 * @param login
	 * 
	 * @return	UserAgent id
